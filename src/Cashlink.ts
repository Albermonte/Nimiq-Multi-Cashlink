...
class Cashlink {
	...
	set value(value: number) {
		if (
			this._value &&
			(this._immutable || this.state !== CashlinkState.UNCHARGED)
		) {
	...
	set fee(fee: number) {
		if (this.state === CashlinkState.CLAIMED) {
			console.warn(
				"Setting a fee will typically have no effect anymore as Cashlink is already claimed",
			);
		}
		this._fee = fee;
	}
	...
	set theme(theme: CashlinkTheme) {
		...
		if (
			!Object.values(CashlinkTheme).includes(theme) ||
			!Nimiq.NumberUtils.isUint8(theme)
		) {
	...
	constructor(
		...
		this._getNetwork().then((network: NetworkClient) => {
			...
			network.on(
				NetworkClient.Events.TRANSACTION_PENDING,
				this._onTransactionReceivedOrMined.bind(this),
			);
			network.on(
				NetworkClient.Events.TRANSACTION_MINED,
				this._onTransactionReceivedOrMined.bind(this),
			);
			network.on(
				NetworkClient.Events.BALANCES_CHANGED,
				this._onBalancesChanged.bind(this),
			);
	...
	public async detectState() {
		...
		if (this.state === CashlinkState.CLAIMED && !balance && !pendingFundingTx)
			return;

		const knownTransactionReceipts = new Map(
			this._knownTransactions.map((tx) => [tx.hash, tx.blockHash!]),
		);

		const transactionHistory = await (
			await this._getNetwork()
		).requestTransactionHistory(address, knownTransactionReceipts);
		this._knownTransactions = this._knownTransactions.concat(
	...
	private async _awaitConsensus(): Promise<void> {
		...
		return new Promise(async (resolve) => {
			const handler = async () => {
				(await this._getNetwork()).off(
					NetworkClient.Events.CONSENSUS_ESTABLISHED,
					handler,
				);
				resolve();
			};
			(await this._getNetwork()).on(
				NetworkClient.Events.CONSENSUS_ESTABLISHED,
				handler,
			);
		});
	...
	private async _sendTransaction(
		...
		await this._awaitConsensus();
		try {
			const proof = Nimiq.SignatureProof.unserialize(
				new Nimiq.SerialBuffer(transaction.proof),
			);
	...
	private async _onTransactionReceivedOrMined(
		transaction: DetailedPlainTransaction,
	): Promise<void> {
		if (
...
namespace Cashlink {
	...
	export const DEFAULT_THEME =
		Date.now() < new Date("Tue, 13 Apr 2020 23:59:00 GMT-12").valueOf()
